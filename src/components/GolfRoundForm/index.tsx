...
const GolfRoundForm = (props: GolfProps) => {
  ...
  const succesMessage = 'Golf round successfully added!';
  const errorMessage = 'There was an error adding a new golf round, please try again';
  ...
  const { register, handleSubmit, formState: { errors }, reset } = useForm<GolfRoundInput>();
  ...
  return (
    <>
      {apiAlert && <APIAlert type={addNewGR?.isError ? 'error' : 'success'} callback={() => setAPIAlert(false)} message={addNewGR?.isError ? errorMessage : succesMessage}/>}
